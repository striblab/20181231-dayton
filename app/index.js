...
function buildGrid(category,parent,target,label,colorClass,symbol,change,xEnd,yMin,yLimit,yFormat,lines,yTick1,yTick2,yTick3,yTick4,yTick5,linecolor1,linecolor2,linecolor3,index) {
    $(parent).append('<div class="smallchart xwidth ' + category + ' ' + lines + '"><div class="label" subject="' + target + '" style="background-color:' + colorScale(change) + ';"><div class="next">' + label + '<div class="stat">' + symbol + '' + d3.format("+.0%")(change) + '</div></div><div class="next expand"><img src="http://static.startribune.com/news/projects/all/20181231-dayton/images/expand.png" /></div></div><div class="chart" id="' + target + '"></div></div>');
}
...
function loadBigChart(target) {
    $("#mainchart").removeClass("noline");
    $("#mainchart").removeClass("halfway");
    $("#mainchart").removeClass("zeroed");
    ...
    for (var i=0; i < data.length; i++) {
        if (target == data[i].target) {
            ...
            $("#mainchart").addClass(data[i].lines);
            break;
        }
    }
}
